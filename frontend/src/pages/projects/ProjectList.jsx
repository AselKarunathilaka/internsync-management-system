...
const ProjectList = () => {
  ...
  const canManageProject = (projectDept) => {
    ...
    if (isEmployee && employeeProfile) {
      ...
      return isGmOrDgm && department === projectDept;
    }
    return false;
  };

  const canDeleteProject = (projectDept) => {
    if (isAdmin) return true;
    if (isEmployee && employeeProfile) {
      const { designation, department } = employeeProfile;
      const isGm = designation === 'General Manager';
      return isGm && department === projectDept;
    }
    return false;
  };

  const canCreateProject = isAdmin || (isEmployee && employeeProfile && (employeeProfile.designation === 'General Manager' || employeeProfile.designation === 'Deputy General Manager'));
      ...
      <div className="animate-fade-in space-y-6">
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
          ...
          <div className="flex gap-3 w-full sm:w-auto">
            {canCreateProject && (
              <Link to="/projects/new" className="btn btn-success flex-1 sm:flex-none shadow-lg">
                + Create Project
              </Link>
            )}
          </div>
                      ...
                      <tr key={project.id} className="hover:bg-indigo-50/50 transition-colors duration-200 animate-fade-in" style={{ animationDelay: `${idx * 50}ms` }}>
                        ...
                        <td className="p-4 text-center">
                          <div className="flex justify-center gap-2">
                            ...
                            {canManageProject(project.department) && (
                            ...
                            {canDeleteProject(project.department) && (
